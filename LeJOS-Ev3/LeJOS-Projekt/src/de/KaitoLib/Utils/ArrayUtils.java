 ...
 *
 * @version	v2.2: Now including Insertion and Selection Sort || Quick Sort still not finished
...
public class  ArrayUtils {
	...
	/**
	 * 	Quicksort Alogrithm: Worst-Case O(n^2), Average- and Best-Case O(n*log(n))
	 * 	NOT FINISHED YET
	 *
	 * 	@param arr	Array
	 * 	@return		Returns @param arr but sorted
	 * */
	...
	public int[] selectionSort(int[] arr)
	{
		for (int i = 0; i < arr.length; i++)
		{
			...
			{
				if (arr[j] < arr[min_index]) swap(arr, j, min_index); min_index = j;
			}
		}
		return arr;
	}
	 ...
	 * 	@return 		Returns arr but sorted
	 * */
	public char[] insertionSort(char[] arr)
	{
		for(int i = 1;i < arr.length; i++) {
			int j = i;
			while(j > 0 && arr[j] < arr[j-1]) {
